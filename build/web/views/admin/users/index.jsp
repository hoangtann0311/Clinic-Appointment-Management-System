                                        ...
                                        <td>
                                            <div class="btn-action-group">
                                                ...
                                                <button class="btn btn-sm btn-outline-secondary btn-action"
                                                        onclick="openEditModal('${u.id}','${fn:escapeXml(u.fullName)}','${fn:escapeXml(u.email)}','${fn:escapeXml(u.phone)}','${u.roleId}','${fn:escapeXml(u.status)}','${not empty u.roleNameDisplay ? fn:escapeXml(u.roleNameDisplay) : fn:escapeXml(roleMap[u.roleId])}','${fn:escapeXml(u.department)}','${fn:escapeXml(u.jobTitle)}')"
            ...
            <form method="post" action="${pageContext.request.contextPath}/admin/users/" autocomplete="off" novalidate>
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-3">
                        ...
                        <%-- Bộ phận / Chức danh: chỉ Quản lý (3) và Nhân viên (4) --%>
                        <div class="col-12 org-fields" id="addOrgFields" style="display: none;">
                            <hr class="my-2 text-muted">
                            <h6 class="fw-bold text-primary mb-3">
                                <i class="bi bi-diagram-3 me-1"></i>Thông tin tổ chức
                            </h6>
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold">Bộ phận</label>
                                    <input type="text" name="department" class="form-control"
                                           maxlength="100" placeholder="VD: Khoa Khám bệnh"
                                           value="${fn:escapeXml(param.department)}">
                                    <div class="form-text">Tối đa 100 ký tự. Để trống nếu chưa phân công.</div>
                                </div>
                                <div class="col-md-6" id="addJobTitleWrap">
                                    <label class="form-label fw-semibold">Chức danh</label>
                                    <input type="text" name="jobTitle" class="form-control"
                                           maxlength="100" placeholder="VD: Nhân viên lễ tân"
                                           value="${fn:escapeXml(param.jobTitle)}">
                                    <div class="form-text">Chỉ áp dụng cho Nhân viên lễ tân.</div>
                                </div>
                            </div>
                        </div>
            ...
            <form method="post" action="${pageContext.request.contextPath}/admin/users/" novalidate>
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-3">
                        ...
                        <%-- Bộ phận / Chức danh: chỉ Quản lý (3) và Nhân viên (4).
                             Hai trường này hiển thị CHỈ-XEM ở trang hồ sơ của họ,
                             nên đây là nơi duy nhất nhập được. --%>
                        <div class="col-12 org-fields" id="editOrgFields" style="display: none;">
                            <hr class="my-2 text-muted">
                            <h6 class="fw-bold text-primary mb-3">
                                <i class="bi bi-diagram-3 me-1"></i>Thông tin tổ chức
                            </h6>
                            <div class="row g-3">
                                ...
                                <div class="col-md-6" id="editJobTitleWrap">
                                    <label class="form-label fw-semibold">Chức danh</label>
                                    <input type="text" name="jobTitle" id="editJobTitle"
                                           class="form-control" maxlength="100"
...
// và AdminUserServlet cũng bỏ qua tham số jobTitle khi vai trò là Quản lý.
function toggleOrgFields() {
    applyOrgVisibility(
        document.querySelector('#addUserModal select[name="roleId"]'),
        document.getElementById('addOrgFields'),
        document.getElementById('addJobTitleWrap'));

    applyOrgVisibility(
        document.getElementById('editRoleId'),
        document.getElementById('editOrgFields'),
        document.getElementById('editJobTitleWrap'));
}

function applyOrgVisibility(roleInput, orgBlock, jobTitleWrap) {
    if (!roleInput || !orgBlock) return;
    var role = String(roleInput.value);
    var isManager = role === '3';
    var isStaff = role === '4';
    orgBlock.style.display = (isManager || isStaff) ? 'block' : 'none';
    if (jobTitleWrap) jobTitleWrap.style.display = isStaff ? 'block' : 'none';
}
...
// ── Edit modal helper ──
function openEditModal(id, fullName, email, phone, roleId, status, roleName, department, jobTitle) {
    ...
    // Đổ sẵn giá trị hiện tại — nếu để trống thì mỗi lần sửa tài khoản
    // sẽ ghi đè bộ phận/chức danh đã gán thành rỗng.
